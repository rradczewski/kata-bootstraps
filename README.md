...
A bootstrap needs to contain a valid [`.devcontainer.json`](./java_junit5/.devcontainer.json) that configures a container with all appropriate tooling. Furthermore, a [`.vscode/tasks.json`](./java_junit5/.vscode/tasks.json) must exist with one task called `"Validate Bootstrap"`, that, when executed, will verify that the test runner correctly runs and reports the single failure present.
